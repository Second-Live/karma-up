...
class Server extends KarmaEventEmitter {
  constructor (config = {}, done) {
    ...
    if (!(config instanceof cfg.Config)) {
      throw new Error('config must be intance of Config, not the object literal. Use parseConfig to get it.')
    }
  ...
  async _start (config, launcher, preprocess, fileList, capturedBrowsers, executor, done) {
    ...
    this.on('browsers_change', () => socketServer.emit('info', capturedBrowsers.serialize()))
    ...
    socketServer.on('connection', (socket) => {
      ...
      const replySocketEvents = events.bufferEvents(socket.emitter, ['start', 'info', 'karma_error', 'result', 'complete'])
      socket.on('error', (err) => this.log.debug('karma server socket error: ' + err))

      socket.emitter.on('register', (info) => {
   ...
   */
  async _close(exitCode) {
    const webServer = this._injector.get('webServer')
    const ws = this._injector.get('socketServer')
    const done = this._injector.get('done')

    ws.close()
    ws.clients.forEach((ws) => ws.terminate())

    let code = 0
    try {
      code = await this.emitExitAsync(exitCode)
    } catch (err) {

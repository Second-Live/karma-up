  ...
  _refresh () {
    ...
    lastCompletedRefresh = Promise.all(
      this._patterns.map(async ({ pattern, type, nocache, isBinary, integrity }) => {
        ...
        const paths = await glob(pathLib.normalize(pattern), { cwd: '/', follow: true, nodir: true, withFileTypes: true })
        ...
        const files = paths
          ...
          .map((file) => new File(file.fullpath(), file.mtime, nocache, type, isBinary))

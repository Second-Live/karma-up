...
exports.mm = (path, pattern, options = {}) => {
  return minimatch(path, pattern, { windowsPathsNoEscape: true, ...options })
}

exports.mmPatternWeight = (pattern) => {
  const m = new Minimatch(pattern, { windowsPathsNoEscape: true })
  if (!m.globParts.length) return [0, 0, 0, 0, 0, 0]
...
exports.mkdirIfNotExists = (directory, done) => {
  mkdir(directory, { recursive: true }, done)
}

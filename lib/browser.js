...
class Browser {
  ...
  execute () {
    this.activeSockets.forEach((socket) => socket.send(JSON.stringify(['execute', this.clientConfig])))
  ...
  bindSocketEvents (socket) {
    ...
    socket.emitter.on('disconnect', (reason) => this.onSocketDisconnect(reason, socket))
    socket.emitter.on('start', (info) => this.onStart(info))
    socket.emitter.on('karma_error', (error) => this.onKarmaError(error))
    socket.emitter.on('complete', (result) => this.onComplete(result))
    socket.emitter.on('info', (info) => this.onInfo(info))
    socket.emitter.on('result', (result) => this.onResult(result))
  }
